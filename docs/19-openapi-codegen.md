...
- **rename / 削除したモデルの残骸が残る**: `openApiGenerate` は出力ディレクトリを掃除せず
  上書きするだけ。スキーマのモデルをリネーム・削除しても、旧名の生成物（例: 旧 `Message.kt`）が
  `generated/openapi/` に残り続ける（gitignore なのでコミットはされないが、死んだコードが
  コンパイル対象に残り紛らわしい）。リネームを含む変更のあとは
  **`./gradlew clean openApiGenerate`** で一掃してから再生成する。
