...
| | `MessageRepository.kt` | リポジトリ **interface**（ドメインが所有・書き込み用） |
...
| | `MessageQueryPort.kt` | 読み取り用 **interface**（application が所有） |
...
| | `ExposedMessageRepository.kt` | `MessageRepository` の実装（書き） |
| | `ExposedMessageQueryPort.kt` | `MessageQueryPort` の実装（読み） |
...
Controller → CreateMessageCommand → CommandHandler(@Transactional)
          → MessageContent(VO で検証) → MessageRepository.create()
...
Controller → ListMessagesQuery → QueryHandler(readOnly)
          → MessageQueryPort → Exposed で直接クエリ → MessageView を返す → DTO へ変換
```
...
1. `foo/domain/` … 集約 `Foo` + VO + `FooRepository`（interface）
2. `foo/application/command/` … `XxxCommand` + `XxxCommandHandler`（更新系ユースケースぶん）
3. `foo/application/query/` … `XxxQuery` + `XxxView` + `FooQueryPort` + `XxxQueryHandler`
4. `foo/infrastructure/` … `Foos`（Exposed Table） + `ExposedFooRepository` + `ExposedFooQueryPort`
