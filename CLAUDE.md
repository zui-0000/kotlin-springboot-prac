...
- `backend/` … Kotlin/Spring アプリ一式（`build.gradle.kts` や `src/` はここ）
- `schema/` … OpenAPI 定義（API 契約・コード生成の起点。スキーマ駆動）
...
アプリ系タスクは `mise.toml` で `dir = "backend"` を指定済みのため、自動で `backend/` 内で走る。
